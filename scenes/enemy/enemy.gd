...
func _ready() -> void:
	$HealthComponent.MAX_HEALTH = scale_factor
	$HealthComponent.health = scale_factor

	$HitboxComponent/CollisionPolygon2D.polygon = shape
	$Line2D.points = shape
	$Line2D.default_color = color
	$Polygon2D.polygon = shape
	$Polygon2D.color = Color(0, 0, 0, 0.5)
	$HealthBar.offset = Vector2(0, -scale_factor-10)
...
func _set_shader_blink_intensity(value: float) -> void:
	var shader_material: ShaderMaterial = $Line2D.material as ShaderMaterial
	shader_material.set_shader_parameter("blink_intensity", value)

func _set_health_bar_alpha(value: float) -> void:
	$HealthBar.modulate.a = value

func _on_local_frame_area_entered(area: Area2D) -> void:
	health_bar.set_value($HealthComponent.health / ($HealthComponent.MAX_HEALTH as float))

	# Lauch VFX
	create_tween().bind_node(self).tween_method(_set_shader_blink_intensity, 1.0, 0, 0.2)	
	create_tween().bind_node(self).tween_method(_set_health_bar_alpha, 1.0, 0.0, 0.5)
	...
	params.from = area.global_position + (area.global_position - global_position).normalized() * scale_factor * 1.1
	params.to = global_position

	var results: Dictionary = space_state.intersect_ray(params)
	EventBus.enemy_hit.emit(self, results)
